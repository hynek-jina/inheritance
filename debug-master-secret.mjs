...
function feistelCipherDecrypt(data, passphrase, iterationExponent, identifier, extendable) {
  ...
  // Data is encrypted as R + L (swapped)
  ...
  // Return L + R to get original order back
...
// Test with both mnemonics
console.log("=== Test 1: Trezor Test Vector ===");
const mnemonic1 = "duckling enlarge academic academic agency result length solution fridge kidney coal piece deal husband erode duke ajar critical decision keyboard";
const masterSecret1 = recoverMasterSecret(mnemonic1, 'TREZOR');
console.log("Mnemonic:", mnemonic1);
console.log("Master secret:", Array.from(masterSecret1).map(b => b.toString(16).padStart(2, '0')).join(''));
console.log("Expected:      bb54aac4b89dc868ba37d9cc21b2cece");
console.log("Match:", Array.from(masterSecret1).map(b => b.toString(16).padStart(2, '0')).join('') === 'bb54aac4b89dc868ba37d9cc21b2cece');

console.log("\n=== Test 2: Your Seed ===");
const mnemonic2 = "guard stay academic academic cylinder swing unhappy deal endless penalty class emphasis gesture away review verify thunder oasis plan triumph";
const masterSecret2 = recoverMasterSecret(mnemonic2, '');
console.log("Mnemonic:", mnemonic2);
console.log("Master secret:", Array.from(masterSecret2).map(b => b.toString(16).padStart(2, '0')).join(''));

console.log("\n=== What Trezor Suite expects ===");
console.log("For your seed to generate address tb1peqvzlxs9etc4pxfe74r0nyf5vsrqwy73zf8cc7aga3pd9rpxxv8q9fe9r2,");
console.log("the master secret would need to be different.");
console.log("");
console.log("Can you check in Trezor Suite what master secret (hex) it shows for your seed?");
console.log("Or can you export the xpub / master fingerprint from Trezor Suite?");