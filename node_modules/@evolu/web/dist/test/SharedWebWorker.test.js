...
test("createSharedWebWorker creates BroadcastChannel and requests lock", () => {
    ...
    expect(mockLocks.request).toHaveBeenCalledWith("evolu-sharedwebworker-test-worker", expect.any(Function));
...
test("createSharedWebWorker handles multiple tabs - first tab becomes owner", async () => {
    ...
    mockLocks.request.mockImplementation((_name, callback) => {
        if (!lockCallback) {
