...
// Wordlist (first 100 words for brevity, complete list would be needed)
...
console.log("\nMaster secret:", Array.from(masterSecret).map(b => b.toString(16).padStart(2, '0')).join(''));

// Create HDKey
const masterKey = HDKey.fromMasterSeed(masterSecret);
console.log("HDKey created");
...
console.log("\nGenerated address:", payment.address);
console.log("Expected address: ", expectedAddress);
console.log("Match:", payment.address === expectedAddress);

// Try with TREZOR passphrase too
console.log("\n=== Testing with TREZOR passphrase ===");
const masterSecretTrezor = recoverMasterSecret(mnemonic, 'TREZOR');
console.log("Master secret (TREZOR):", Array.from(masterSecretTrezor).map(b => b.toString(16).padStart(2, '0')).join(''));

const masterKeyTrezor = HDKey.fromMasterSeed(masterSecretTrezor);
const childTrezor = masterKeyTrezor.derive(path);
const internalKeyTrezor = Buffer.from(childTrezor.publicKey);
const paymentTrezor = bitcoin.payments.p2tr({
  internalPubkey: internalKeyTrezor.slice(1, 33),
  network: TESTNET_NETWORK,
});
console.log("Address (TREZOR):", paymentTrezor.address);