...
function rs1024CreateChecksum(customization: string, data: number[]): number[] {
  ...
  return [(polymod >> 20) & 0x3ff, (polymod >> 10) & 0x3ff, polymod & 0x3ff];
}
...
function decodeMetadata(wordIndices: number[]): ShareMetadata {
  const identifier =
    ((wordIndices[0] << 5) | ((wordIndices[1] >> 5) & 0x1f)) & 0x7fff;
  ...
  const groupCount =
    (((wordIndices[2] & 0x03) << 2) | ((wordIndices[3] >> 8) & 0x03)) + 1;
...
function decodeShare(words: string[]): {
  metadata: ShareMetadata;
  encryptedSecret: Uint8Array;
} {
  ...
  if (wordIndices.length !== 20 && wordIndices.length !== 33) {
    throw new Error(
      `Invalid mnemonic length: ${wordIndices.length} words. Expected 20 or 33.`,
    );
  }
...
export function generateMnemonic(masterSecret: Uint8Array): string {
  if (masterSecret.length !== 16 && masterSecret.length !== 32) {
    throw new Error(
      "Master secret must be 16 bytes (128-bit) or 32 bytes (256-bit)",
    );
  }
  ...
  const encryptedSecret = feistelCipherEncrypt(
    masterSecret,
    passphrase,
    iterationExponent,
    identifier,
...
export function recoverMasterSecret(mnemonic: string): Uint8Array {
  ...
  const masterSecret = feistelCipherDecrypt(
    encryptedSecret,
    passphrase,
    iterationExponent,
    identifier,
