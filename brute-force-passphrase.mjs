...
// Brute force common passphrases
...
const expectedFingerprint = '851fc673';

const testPassphrases = [
  '', 'TREZOR', 'trezor', 'guard', 'password', '123456', 
  'testnet', 'test', 'bitcoin', 'btc', 'wallet', 'seed',
  'stay', 'academic', 'cylinder', 'swing'
];

console.log("=== Brute forcing passphrase ===");
...
for (const passphrase of testPassphrases) {
  try {
    const masterSecret = recoverMasterSecret(mnemonic, passphrase);
    const masterKey = HDKey.fromMasterSeed(masterSecret);
    const fingerprint = Array.from(masterKey.fingerprint).map(b => b.toString(16).padStart(2, '0')).join('');
    
    if (fingerprint === expectedFingerprint) {
      console.log(`✓ Found! Passphrase: "${passphrase}" -> Fingerprint: ${fingerprint}`);
    } else {
      console.log(`  "${passphrase}" -> ${fingerprint}`);
    }
  } catch (e) {
    console.log(`  "${passphrase}" -> Error: ${e.message}`);
  }
}

console.log("\nIf none match, Trezor might be using a different configuration.");